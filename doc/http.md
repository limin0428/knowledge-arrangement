...
> 强缓存：直接使用浏览器缓存，不会请求服务器

> 协商缓存：请求浏览器，查看是否缓存失效，未失效返回304，使用浏览器缓存

对比 | 强缓存 | 协商缓存 
:---: |:---:| :---: 
缓存存放位置 | 本地浏览器 | 本地浏览器
http状态码 | 200 | 304
谁来决定 <br> (优先级递减) | Pragma <br> Cache-Control <br> Expires | ETag/if-Not-Match <br> Last-Modified/if-Modified-Since 
是否有效(操作) | Ctrl+F5 强制刷新 -- 无效 <br> F5刷新 -- 无效 <br> 地址栏访问 -- 有效  <br> 页面链接跳转 -- 有效 <br> 新开窗口 -- 有效  <br> 前进/后退 -- 有效 | Ctrl+F5 强制刷新 -- 无效 <br> F5刷新 -- 有效 <br> 地址栏访问 -- 有效  <br> 页面链接跳转 -- 有效 <br> 新开窗口 -- 有效  <br> 前进/后退 -- 有效

##### 6-1.强缓存
> 强缓存：强缓存在数据未失效的情况下(即Cache-Control的max-agemax-age没有过期或者Expires的缓存时间没有过期)，直接使用浏览器的缓存数据，不会向服务端请求。所以它的加载速度最快，性能最好。但是在此期间服务端的资源修改了，会出现无法即时更新。解决方案：Ctrl + F5强制刷新，使强缓存无效。

Header属性 | 可选值 | 优先级 | 优缺点 
:---: |:---:| :---: | :---: |
Pragma | no-cache: 不直接使用缓存，根据新鲜度来判断 | 高 | 1.响应头不支持该属性 <br> 2.在HTTP/1.1中已废弃
Cache-Control | no-cache: 不直接使用缓存，根据新鲜度来判断 <br> no-store: 不缓存(强/协商都不走) <br> max-age: 缓存有效时长 <br> must-revalidate: 每次访问需要缓存校验 | 中 | 1.请求头和响应头都支持该属性
Expires | GTM时间 | 低 | 1.服务端时间和客户端时间不一致会出现问题

##### 6-2.协商缓存
> 协商缓存：当强缓存失效时，或者设置不走强缓存时，浏览器就会与服务端对比资源是否更新。如果没有更新，返回状态码304，使用浏览器缓存

Header属性 | 可选值 | 优先级 | 优缺点 
:---: |:---:| :---: | :---: |
ETag/if-Not-Match | 校验值 | 高 | 1.默认使用hash算法生成ETag值，在分布式环境中可能会出现不同服务器生成的ETag值不一致 <br> 2.精确的判断资源是否被修改 <br> 3.计算Etag需要性能消耗
Last-Modified/if-Modified-Since | GTM时间 | 中 | 1.只要资源修改，无论内容是否变化，都会将资源返给客户端 <br> 2.以时刻为标识，无法获取同一秒内的修改
